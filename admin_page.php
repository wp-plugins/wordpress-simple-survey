...
<div class="wrap">
	<?php echo "<h2>" . __( 'Wordpress Simple Survey Setup', 'wpss_trdom' ) . "</h2>"; ?>
	...
	<form name="wpss_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<h2><?php _e("Simple Survey Options: " ); ?></h2>
		...
		<p><?php _e("<strong>Number Of Questions:</strong> " ); ?><input type="text" name="wpss_numQuestions" value="<?php echo stripslashes($numQuestions); ?>" size="2" maxlength="2" /><input type="submit" name="Add Question" value="<?php _e('Update', 'wpss_trdom' ) ?>" /></p>

		<p><?php _e("<strong>Send Results to Email Address:</strong> " ); ?><input type="text" name="wpss_sendEmail" size="40" value="<?php echo stripslashes($sendEmail); ?>" /><br />Leave blank to send no email result.</p>
		...
		<p><strong>Scoring:</strong><br /> Sum( QuestionWeight_n * AnswerWeight_n )<br /></p>
		...
		<p class="submit">
		<input type="submit" name="Submit" value="<?php _e('Update Options', 'wpss_trdom' ) ?>" />
		</p>
	</form>
</div>
